		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "v")
				{
					...
					if (spos.size() > 3) {
						cpos.X = std::stof(spos[3]) / 255.0f;
						cpos.Y = std::stof(spos[4]) / 255.0f;
						cpos.Z = std::stof(spos[5]) / 255.0f;
						Colors.push_back(cpos);
					}
				...
				if (algorithm::firstToken(curline) == "f")
				{
					...
					GenVerticesFromRawOBJ(vVerts, Positions, TCoords, Normals, Colors, curline);
